...
# obtenemos Dogecoin en Bitcoin y luego se convierte en usd 
...
# obtenemos datos de Bitcoin, Litecoin, Cardano en dolares
...
# 2 justamos las bases manteniendo únicamente las columnas donde hay datos tanto de dogecoin como de bitcoin
# para esto se agrupa los datos por fecha y hora, luego se crea una columna "crypto" que toma valores n = 1 
# o n = 2, siendo 1 cuando la cotización en esa fecha y hora es solo de una de las criptomonedas, y 2 cuando
# en esa fecha y hora se transaron ambas criptomonedas, luego se filtra la base para n=2 y de esa manera se 
# obtiene una base que contiene transacciones en fecha y hora exactamente iguales 
# left_join(x, y, by='Flag') %>% left_join(., z, by='Flag') 
